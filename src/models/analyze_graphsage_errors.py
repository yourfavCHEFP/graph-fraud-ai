...
Uses the exact feature mapping and normalization statistics
...
def load_checkpoint():
    ...
    if missing:
        ...
        raise ValueError(
            "GraphSAGE checkpoint is missing "
            "required fields: " + ", ".join(missing) + ". Retrain GraphSAGE using the "
    ...
    if normalization_mean.shape[1] != checkpoint["input_dim"]:

        raise ValueError(
            "Normalization mean dimension " "does not match model input dimension."
        )

    if normalization_std.shape[1] != checkpoint["input_dim"]:
...
def generate_predictions(
    ...
    y_prob = probability[test_mask].cpu().numpy()
...
def analyze_feature_groups(
    ...
    if test_features.shape[1] != len(feature_names):
        ...
        raise ValueError(
            ...
            f"Test feature matrix contains "
            f"{test_features.shape[1]} columns.\n"
            f"Feature names contain "
...
def main():
    ...
    if enhanced_features.shape[1] != len(feature_names):
        ...
        raise ValueError(
            "The feature builder and GraphSAGE "
            "checkpoint do not use the same "
            "number of features."
        )

    # --------------------------------------------------------
    # NORMALIZE USING CHECKPOINT
    # --------------------------------------------------------
    ...
    graph.x = (enhanced_features - mean) / std
